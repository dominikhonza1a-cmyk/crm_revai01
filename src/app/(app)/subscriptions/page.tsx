...
type EditState = { id?: string; name: string; purpose: string; email: string; password: string; url: string; amount: string; currency: "USD" | "EUR" | "CZK" | "GBP"; period: "monthly" | "yearly"; notes: string } | null;

const EMPTY: NonNullable<EditState> = { name: "", purpose: "", email: "", password: "", url: "", amount: "", currency: "USD", period: "monthly", notes: "" };
...
export default function SubscriptionsPage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      currency: edit.currency, period: edit.period, notes: edit.notes || undefined,
    };
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted">Vše, co platíme — sdílené mezi vámi. Částky se přepočítávají na CZK denním kurzem ČNB; roční platby se počítají jako 1/12 měsíčně.</p>
        <button className={btnPrimary} onClick={() => setEdit({ ...EMPTY })}>+ Nové předplatné</button>
      </div>
      ...
      {!items.length ? (
        ...
        <Card>
          <SectionTitle right={<span className="font-display text-xl tracking-wide text-ink">{money(Number(list.data!.totalMonthlyCzkMinor))} <span className="text-xs font-sans tracking-normal text-faint">/ měsíc celkem</span></span>}>
            Fixní náklady
          </SectionTitle>
              ...
              <tbody className="divide-y divide-line">
                {items.map((s) => (
                  <tr key={s.id} className="hover:bg-white/5">
                    ...
                    <td className="py-2.5 pr-3 text-right text-muted">{(Number(s.amountMinor) / 100).toLocaleString("cs-CZ")} {s.currency}{s.period === "yearly" ? "/rok" : "/měs"}</td>
                    <td className="py-2.5 pr-3 text-right font-medium text-ink">{money(Number(s.monthlyCzkMinor))}</td>
                    <td className="py-2.5 text-right">
                      <button className="text-xs text-faint hover:text-accent" onClick={() => setEdit({
                        ...
                        url: s.url ?? "", amount: String(Number(s.amountMinor) / 100), currency: s.currency as never, period: s.period as never, notes: s.notes ?? "",
                      })}>Upravit</button>
      ...
      {edit && (
        <Modal title={edit.id ? "Upravit předplatné" : "Nové předplatné"} onClose={() => setEdit(null)}>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div><label className={fieldLabel}>Období</label>
                  <select className={fieldInput} value={edit.period} onChange={(e) => setEdit({ ...edit, period: e.target.value as never })}>
                    <option value="monthly">měsíčně</option><option value="yearly">ročně</option>
                  </select></div>
              </div>
